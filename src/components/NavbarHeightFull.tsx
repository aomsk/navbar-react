...
export default function NavbarHeightFull() {
  const links = [
    { name: "Home", link: "/" },
    { name: "About", link: "/about" },
    { name: "Services", link: "/skills" },
    { name: "Our Teame", link: "/projects" },
  ...
  return (
    <nav className="shadow-md w-full fixed top-0 left-0 z-[1]">
      <div className="md:flex items-center justify-between bg-white py-4 md:px-10 px-7">
        ...
        <ul
          className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 h-screen md:h-auto transition-all duration-500 ease-out ${
            open ? "top-10 opacity-100" : "top-[-490px] opacity-0"
          } md:opacity-100`}
        >
          {links.map((itme) => (
            <li key={itme.name} className="md:ml-8 text-md font-bold md:my-0 my-7">
